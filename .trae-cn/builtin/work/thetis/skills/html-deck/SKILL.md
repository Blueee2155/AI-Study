...
⛔ **Slide count refers to content slides only.** When the user specifies a number (e.g. "10 slides"), that count refers to **content slides only** — cover, section-divider/transition, and thanks/ending pages are **not** included in that number and should be added on top.

⛔ **Language consistency is mandatory.** The plan (`plan.md`), all slide text, speaker notes, and any other deliverable content **must** use the same language as the user's query. For example, if the user writes in Chinese, the entire deck — titles, body text, notes — must be in Chinese. Only deviate if the user explicitly requests a different language.
...
Write a `plan.md` file with two sections. **`plan.md` is an intermediate planning artifact only — it must NOT appear in the final deliverables presented to the user.**
...
2. **Generate images with GenerateImage** — call GenerateImage directly (no subagent needed). Generate each asset one by one, saving them to the deck's `assets/` directory:
   ...
   - Choose the right `image_size` for the target aspect ratio:
     - `landscape_16_9` (1024×576) — full-slide backgrounds, wide layouts
     - `landscape_4_3` (1024×768) — content area illustrations
     - `square_hd` (1024×1024) — headshots, profile images, square icons
   - Save as `assets/{name}_{w}x{h}.png` (e.g. `assets/hero_1024x576.png`)
...
1. Scaffold the deck directory:
   ...
   **Linux / macOS:**
   ```bash
   bash ./scripts/new-deck.sh my-talk <work-dir>
   ```

   **Windows (native PowerShell):**
   ```powershell
   pwsh -File ./scripts/new-deck.ps1 my-talk <work-dir>
   ```

   This creates `<work-dir>/my-talk/` with:
   ```
   my-talk/
...
2. Copy the chosen theme into `_shared/themes/`:
   ```bash
   cp <skill-dir>/assets/themes/<theme>.css <work-dir>/my-talk/_shared/themes/
   ```
...
6. Add `data-anim` / `data-fx` animations sparingly; copy needed fx scripts:
   ```bash
   cp <skill-dir>/assets/animations/fx/<effect>.js <work-dir>/my-talk/_shared/animations/fx/
   ```
...
```bash
open <work-dir>/my-talk/my-talk.html
```

Preview the HTML file (which uses relative paths to `./_shared/` and `./assets/`) in any browser.

- **O** — overview grid
- **T** — cycle themes
- **S** — open presenter window
- **E** — toggle edit mode (edit text directly in browser)
- **N** — notes drawer

### Step 6: Deploy & Deliver

⚠️ **Always deploy before delivering to the user.** This copies the deck folder to the user's workspace, removes intermediate files (plan.md), and cleans unreferenced assets.

**Linux / macOS:**
```bash
bash ./scripts/deploy.sh <work-dir>/my-talk <workspace-dir>
```

**Windows (native PowerShell):**
```powershell
pwsh -File ./scripts/deploy.ps1 <work-dir>/my-talk <workspace-dir>
```

This copies the entire `my-talk/` folder to `<workspace-dir>/my-talk/`, then:
1. Removes `.md` files (plan.md etc.)
2. Removes unreferenced themes from `_shared/themes/`
3. Removes unreferenced fx scripts from `_shared/animations/fx/`
4. Removes unreferenced shared CSS/JS from `_shared/`
5. Verifies all HTML files contain `<!-- Generated by Trae Work -->` on line 1

Final deliverable structure:
```
<workspace-dir>/my-talk/
├── my-talk.html            (must contain <!-- Generated by Trae Work -->)
├── assets/                 (images, chart JS — only what's used)
└── _shared/                (only referenced assets remain)
    ├── base.css
    ├── fonts.css
    ├── runtime.js
    ├── ...
    ├── themes/<theme>.css
    └── animations/fx/<effect>.js
```

## ⚠️ HTML IS the final deliverable

The HTML deck **is** the presentation itself — not an intermediate format. Do NOT convert or export it to PowerPoint (.pptx), Keynote, PDF, or any other third-party format. HTML provides:

- Pixel-perfect rendering across all platforms (any modern browser)
- Interactive animations, Canvas FX, and ECharts that cannot survive format conversion
- Theme switching, presenter mode, and edit mode built in

**Delivery:** The deployed folder in the user's workspace is the final product. Users open `<name>.html` in a browser to present — no additional software required. Recommend pressing `F` to enter fullscreen mode for the intended viewing experience.
...
├── scripts/
│   ├── new-deck.sh                (scaffold a deck directory with _shared/)
│   ├── new-deck.ps1               (scaffold — Windows PowerShell version)
│   ├── deploy.sh                  (deploy deck folder to user workspace)
│   ├── deploy.ps1                 (deploy — Windows PowerShell version)
...
```
←  →  Space  PgUp  PgDn  Home  End    navigate (disabled in edit mode)
...
Esc                                     exit edit mode / close overlays
#/N in URL                              deep-link to slide N
?preview=N                              preview-only mode (single slide)
```
