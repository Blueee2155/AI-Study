...
Preview the original `index.html` (which uses relative paths back to the shared `assets/` directory) in any browser.

Checklist:
- [ ] **O** (overview) — scan all slides for visual balance, no text overflow
- [ ] **T** (theme) — cycle 2-3 themes, check contrast and readability
- [ ] **S** (presenter) — verify notes appear, timer works
- [ ] **E** (edit) — test text editing, fix any typos directly
- [ ] **← →** — navigate through all slides, check animations
- [ ] **F** (fullscreen) — verify 16:9 layout fills screen properly

## Step 6: Bundle & Deliver
