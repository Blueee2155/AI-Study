...
- **Windows (PowerShell):**
  ```powershell
  powershell -File <skill-dir>/scripts/new-report.ps1 <report-name> <output-dir>
  ```
- **macOS/Linux:**
  ```bash
...
│   └── render_plantuml.py
├── scripts/
│   ├── new-report.sh        (scaffold a report directory — macOS/Linux)
│   └── new-report.ps1       (scaffold a report directory — Windows)
```
