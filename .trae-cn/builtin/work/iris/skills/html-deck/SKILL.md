...
1. Scaffold:
   - **Windows (PowerShell):** `powershell -File ./scripts/new-deck.ps1 my-talk <workspace-dir>`
   - **macOS/Linux:** `bash ./scripts/new-deck.sh my-talk <workspace-dir>`
...
├── scripts/
│   ├── new-deck.sh                (scaffold a deck directory with _shared/ — macOS/Linux)
│   ├── new-deck.ps1               (scaffold a deck directory with _shared/ — Windows)
│   ├── deploy.sh                  (legacy — not used; deck is built directly in workspace)
│   ├── deploy.ps1                 (Windows PowerShell version of deploy.sh)
│   ├── render.sh                  (headless Chrome → PNG — macOS/Linux)
│   └── render.ps1                 (headless Chrome → PNG — Windows)
```
